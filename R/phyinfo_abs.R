...
for (prt in 1:length(sets[,1])){
  ...
  for (partition in 1:length(parts)){
    ...
    for (t2a in seq(0,1,0.01)){
      rangeCoord = as.numeric(unlist(strsplit (sets[[2]][[2]][prt], '-')))
      minCoord = rangeCoord[1]
      maxCoord = rangeCoord[2]
      phyinf = PhyInformR:::site.summer(mxrates[minCoord:maxCoord,3][parts[[partition]]],t2a)
      t2b <- data.frame(x1=t2a,
                        y1=phyinf,
                        y1Std = phyinf/length(mxrates[minCoord:maxCoord,3][parts[[partition]]]))
      fit1 <- rbind(fit1, t2b)
    }
